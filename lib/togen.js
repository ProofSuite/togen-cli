...
class Togen {
  ...
  async interactive() {
    let configured = await config.contracts.settingsFileExist()

    if (configured) {
      let fileSettings = Options.parseSettings(config.contracts.settingsFile)
      if (!fileSettings) throw new AppError('Could not load configuration')


      let settings = new Options(fileSettings)
      this.router.openMainMenu({settings})
    }
    else {
