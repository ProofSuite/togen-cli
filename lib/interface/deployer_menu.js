...
class DeployerMenu {
  ...
  constructor({router, settings}) {
    this.router = router
    this.settings = settings || settings.getLocal()
  }
  ...
  async showDeployerMenu() {
    ...
    let menu = questions.deployerConfigurationMenu({additionalFields: ['Back','Close']})
  ...
  async showDefaultNetworkAndWalletForm() {
    var [err, { network }] = await until(command.prompt(questions.defaultNetworkForm))
    if (err) throw err;
    ...
    if (wallets.length == 0) {
      ...
      var  [err, { defaultAddress }] = await until(command.prompt(form))
      if (err) throw err;

      config.user.updateDefaultNetwork(network)
      config.user.updateDefaultWallet(defaultAddress)
  ...
  async showDeployContractForm(contract) {
    ...
    var [error, txHash] = await until(deployment.start())
    if (error) console.log(error)

    await display.deploymentSuccess(txHash)
  }
