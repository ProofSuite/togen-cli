...
class Interface {
  ...
  async showMainMenu() {
    ...
    if (choice === 'Configure Contracts') {
      ...
      display.currentConfiguration(this.settings);
  ...
  async showContractConfigurationMenu({}) {
    ...
    const choice = await this.showContractMenu({ additionalFields: ['Display Contract Configuration', 'Go to main menu'] });
  ...
  async showTokenSaleMenu() {
    ...
    if (choice === 'Base Configuration') {
      ...
      await this.settings.tokenSale.updateTimed();
      await this.showTokenSaleMenu();
