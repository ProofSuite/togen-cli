...
class Account {
  ...
  static async deleteWallet() {
    let wallets = Account.walletObjects()
    let walletList = questions.deleteWallet(wallets)
    const { name, network } = await command.prompt(questions.destroyWallet)

    let wallet = Account.findWalletByName(wallet)
    if (!wallet) console.log("Wallet not found")

    var [err] = await until(Wallet.deleteFile(wallet))
    config.user.deleteWalletRecord(wallet.filePath)
