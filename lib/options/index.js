...
class Options {
    ...
    this.includedContracts = {
      token: isNonEmptyObject(settings.token),
      tokenSale: isNonEmptyObject(settings.tokenSale),
      presale: isNonEmptyObject(settings.presale),
      presaleToken: isNonEmptyObject(settings.presaleToken),
      wallet: isNonEmptyObject(settings.wallet)
    }
  }
  ...
  load() {
    let json = config.localSettingsFile()
